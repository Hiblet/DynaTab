...
fc.utils.removeEventHandler = function (elem, eventType, handler) {
...
// NOTE: Before calling this function, event handlers such as onclick handlers
// should be removed using fc.utils.removeEventHandler.  Modern browsers 
// should be able to cope with unreferenced event handlers, but it is better 
// to remove them consciously first.
fc.utils.removeElement = function (node) {
...
fc.utils.swapCSSClass = function (sCurrentClass, sClassToRemove, sClassToAdd) {
...
///////////////////////////////////////////////////////////////////////////////
//
// Colour Value Manipulation 
// See: http://www.sitepoint.com/javascript-generate-lighter-darker-color/

// Arguments:
//  hex: 3 or 6 digit hex colour value, with or without hash
//  lum: Number between 1 and -1, where -0.1 is "10% darker" and +0.2 is "20% lighter"
// Returns:
//  string; 
//  If bReturnHex is not set, or true:
//              Hex colour string ie "#c9c9dd"
//  If bReturnHex is false:
//              RGB colour string ie "rgb('12','53','235')"
fc.utils.hexColourLuminance = function (hex, lum, bReturnHex) {
    lum = lum || 0;
    ...
    for (var i = 0; i < 3; i++) {
        ...
        arrRGBValues.push(colour);
    }

    return fc.utils.rgbArrayColourLuminance(arrRGBValues, lum, bReturnHex);
}

// Arguments:
//  rgbString: "rgb('12','65','233')" (as returned when reading computed style value)
//  lum: Number between 1 and -1, where -0.1 is "10% darker" and +0.2 is "20% lighter"
// Returns:
//  string; 
//  If bReturnHex is not set, or true:
...
//  If bReturnHex is false:
//              RGB colour string ie "rgb('12','53','235')"
fc.utils.rgbStringColourLuminance = function (rgbString, lum, bReturnHex) {

    var arrStringRGBvalues = rgbString.match(/\d+/g);

    var arrRGBvalues = [];
    for (var i = 0; i < arrStringRGBvalues.length; ++i) {
        var sColour = arrStringRGBvalues[i];
        var colour = parseInt(sColour, 10) || 0;
        arrRGBvalues.push(colour);
    }
...
// Arguments:
//  arrRGBValues: Array of three int values between 0 and 255, eg [12,65,233]
//  lum: Number between 1 and -1, where -0.1 is "10% darker" and +0.2 is "20% lighter"
// Returns:
//  string; 
//  If bReturnHex is not set, or true:
...
//  If bReturnHex is false:
//              RGB colour string ie "rgb('12','53','235')"
fc.utils.rgbArrayColourLuminance = function (arrRGBvalues, lum, bReturnHex) {

    // default syntax
    bReturnHex = (typeof bReturnHex === 'undefined') ? true : bReturnHex;

    if (bReturnHex) {
        return fc.utils.rgbArrayColourLuminanceHex(arrRGBvalues, lum);
    }
    else {
        return fc.utils.rgbArrayColourLuminanceRGB(arrRGBvalues, lum);
    }
}

fc.utils.rgbArrayColourLuminanceHex = function (arrRGBvalues, lum) {
    lum = lum || 0;
    var sReturn = "#";

    for (var i = 0; i < arrRGBvalues.length; ++i) {
        var colour = arrRGBvalues[i];
        var colour1 = fc.utils.adjustColourLuminance(colour, lum);
        var sHexColour = colour1.toString(16);
        sReturn += ("00" + sColour).substr(sColour.length);
    }

    return sReturn;
}

fc.utils.rgbArrayColourLuminanceRGB = function (arrRGBvalues, lum) {
    lum = lum || 0;
    var sReturn = "rgb(%)";

    var arrStringColoursAdjusted = [];
    for (var i = 0; i < arrRGBvalues.length; ++i) {
        var colour = arrRGBvalues[i];
        var colour1 = fc.utils.adjustColourLuminance(colour, lum);
        var sColourAdjusted = colour1.toString();
        arrStringColoursAdjusted.push(sColourAdjusted);
    }

    sReturn = sReturn.replace("%", arrStringColoursAdjusted.join(","));
    return sReturn;
}

// Arguments:
//  colour; int value between 0 and 255
//  lum: Number between 1 and -1, where -0.1 is "10% darker" and +0.2 is "20% lighter"
fc.utils.adjustColourLuminance = function (colour, lum) {
    return Math.round(Math.min(Math.max(0, colour + (colour * lum)), 255));
}
